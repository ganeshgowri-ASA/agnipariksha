...
export default async function SparePartDetailPage({ params }: PageProps): Promise<React.ReactElement> {
  const { id } = await params;
  return (
    <main className="min-h-screen bg-gray-950 text-gray-100 px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-2xl space-y-4">
        <h1 className="text-xl font-bold">Spare part · <span className="font-mono text-orange-300">{id}</span></h1>
        <p className="text-sm text-gray-400">
          Scanned from /scan. Print the QR label to attach to this spare part.
        </p>
        <div className="flex flex-wrap gap-2">
          <a
            href={`/spare-parts/${encodeURIComponent(id)}/label`}
            target="_blank"
            rel="noreferrer"
