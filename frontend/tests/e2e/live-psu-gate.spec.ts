...
// PR #54 wired MSW in via /mockServiceWorker.js. Once that SW boots, all
// `fetch()` calls inside the page are proxied through the SW, and the
// network requests Playwright sees originate from the SW context — which
// `page.route()` does not intercept. Blocking service workers for this
// suite keeps every fetch on the main page and lets the route stubs win.
