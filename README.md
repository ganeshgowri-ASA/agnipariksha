...
### Threaded per-Module assistant

A single AI thread spans **Setup → Live Monitor → Data → Analysis → Report**
across all 7 IEC test tabs, keyed by the Module ID entered in the header.
Available in the right-hand rail of every test tab and full-screen on the
`/AI` tab.

Backend tool calls (server-resolved before synthesis):

- `get_run` — fetch a TestRun by id or recent runs for the module.
- `query_telemetry` — sample window for a run (chronological, capped at 1000).
- `recompute_analysis` — recompute Gate-2 ΔPmax against telemetry.
- `suggest_pass_fail` — recommendation + confidence + reason.

API endpoints:

- `GET    /api/assistant/threads/{module_id}` — load thread + history.
- `POST   /api/assistant/threads/{module_id}/messages` — SSE stream.
- `DELETE /api/assistant/threads/{module_id}` — wipe the transcript.
- `GET    /api/assistant/tools` — discoverable tool catalogue.

Responses stream as Server-Sent Events: `meta` → `tool_call` → `tool_result`
→ `token`* → `done`. The deterministic local router runs offline; when
`ANTHROPIC_API_KEY` is set the narrative is regenerated by Claude with the
tool results pre-injected so numbers can't drift.
